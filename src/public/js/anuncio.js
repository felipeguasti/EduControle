...
function carregarDetalhesReserva(idReserva) {
    ...
    function editarAnuncio(id) {
        ...
        fetch(`/admin/anuncios/listar/${id}`)
        ...
        .then(anuncio => {
            ...
            const formAnuncio = document.getElementById('formAnuncio');
            formAnuncio.action = `/admin/anuncios/listar/${id}`;
    ...
    function excluirAnuncio(id) {
        if (confirm('Tem certeza que deseja excluir este anúncio?')) {
            fetch(`/admin/anuncios/listar/${id}`, { method: 'DELETE' })
            .then(response => {
                if (response.ok) {
    ...
    if (loadSectionButton) {
        loadSectionButton.addEventListener('click', function() {
